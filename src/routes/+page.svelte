...
<div class="flex flex-col h-screen w-screen">
  ...
  <div class="flex-1 overflow-y-auto p-4 bg-gradient-to-b from-[#1E3345] to-transparent mt-10 ml-10 mr-10 rounded-3xl">
    {#each convos as convo}
      <div class="mb-4 {convo.type === 'user' ? 'text-right' : ''}">
        <p class="text-gray-300 px-4 py-2 bg-[#0A2F47] rounded-xl inline-block {convo.type === 'user' ? '' : 'bg-blue-900'}">
          {convo.text}
        </p>
      </div>
    {/each}
